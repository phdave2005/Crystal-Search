...
function LabelElement(props) {
	...
	return (
		<label htmlFor={props.labelFor} data-testid="label"><span className="default-label"><span dangerouslySetInnerHTML={{__html: props.text}}></span><span className={!!props.tooltip ? '' : 'DN'}><FontAwesomeIcon data-tooltip-id={tooltipId} data-tooltip-content={tooltipContent} className="ML4" icon={faCircleQuestion} /></span></span><span className="error-label DN"><FontAwesomeIcon icon={faTimes} /><span className="error-label-text ML4"></span></span><Tooltip id={tooltipId} /></label>
	);
}
